    ...
    const handleSubmit = e => {
        ...
        if (address.value === "") {
            ...
            const order = {
                ...
                packageName: pg?.name,
                packagePrice: pg?.price,
                orderDate: `${date}/${month}/${year}`
            }
            if (!booked) {
                axios.post("https://secret-basin-49124.herokuapp.com/addorder", order)
                    .then(res => {
                        setBooked(true);
                        handleResetInput(e);
                        // console.log(res);
    ...
    return (
        <div className='form-container booking-form'>
            <form onSubmit={handleSubmit}>
                ...
                <input readOnly disabled type="text" value={user?.displayName} name="username" id="username" />
